...
void Plot_ctti(const Tree& t)
{
	...
	auto [v_jpn, v_eng] = t | Filter(exam == 0) | ToVector(cast_f64(jpn), cast_f64(eng));
	...
	matplot::xlabel("jpn");
	matplot::ylabel("eng");
	matplot::save(std::format("scatter_jpn_eng_ctti_{}.png", tree_type));

	std::vector<double> v_math = t | Filter(number == 0) | ToVector(cast_f64(math));
	matplot::hist(v_math);
	matplot::title("math score distribution");
	matplot::xlabel("math");
	matplot::save(std::format("hist_math_ctti_{}.png", tree_type));
