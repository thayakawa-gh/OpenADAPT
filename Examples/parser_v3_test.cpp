...
int main()
{
	...
	std::cout << "=== OpenADAPT Parser V3 Test ===" << std::endl << std::endl;
	std::cout << "重要な改善点:" << std::endl;
	std::cout << "1. RttiFieldNode を直接渡す（パフォーマンス最適化）" << std::endl;
	std::cout << "2. リテラルサポート（整数、浮動小数点、文字列）" << std::endl;
	std::cout << "3. 型接尾辞サポート（i08, i16, i32, i64, f32, f64）" << std::endl << std::endl;
	
	// 例1: リテラルサポート - デフォルト型
	{
		std::cout << "式: x + 10 (整数リテラル、デフォルトI32)" << std::endl;
	...
	// 例2: リテラルサポート - 浮動小数点
	{
		std::cout << "式: x + 3.5 (浮動小数点リテラル、デフォルトF64)" << std::endl;
	...
	// 例3: 型接尾辞 - i64
	{
		std::cout << "式: x + 100i64 (I64型指定)" << std::endl;
		auto lambda = ParseRttiFuncNode(tree, "x + 100i64");
		std::cout << "  tree[0]: " << lambda(tree, Bpos{ 0 }).i64() << " (期待値: 110)" << std::endl;
		std::cout << "  tree[1]: " << lambda(tree, Bpos{ 1 }).i64() << " (期待値: 105)" << std::endl;
		std::cout << "  ✓ 成功" << std::endl << std::endl;
	}
	
	// 例4: 型接尾辞 - f32
	{
		std::cout << "式: y * 2.5f32 (F32型指定)" << std::endl;
		auto lambda = ParseRttiFuncNode(tree, "y * 2.5f32");
		std::cout << "  tree[0]: " << lambda(tree, Bpos{ 0 }).f32() << " (期待値: 50.0)" << std::endl;
		std::cout << "  tree[1]: " << lambda(tree, Bpos{ 1 }).f32() << " (期待値: 37.5)" << std::endl;
		std::cout << "  ✓ 成功" << std::endl << std::endl;
	}
	
	// 例5: 複合式 - 演算子の優先順位
	{
		std::cout << "式: x + y * 2 (リテラルを含む複合式)" << std::endl;
	...
	std::cout << "=== 階層関数 + リテラル ===" << std::endl << std::endl;
	...
	// 例7: 階層関数 + リテラル
	{
		std::cout << "式: mean(score) + 10 (階層関数とリテラルの組み合わせ)" << std::endl;
	...
	// 例9: exist with condition containing literal
	{
		std::cout << "式: exist(score > 90) (条件付き階層関数)" << std::endl;
		auto lambda = ParseRttiFuncNode(htree, "exist(score > 90)");
		std::cout << "  Tanaka: " << (int)lambda(htree, Bpos{ 0 }).i08() << " (期待値: 0)" << std::endl;
		std::cout << "  Sato: " << (int)lambda(htree, Bpos{ 1 }).i08() << " (期待値: 1)" << std::endl;
		std::cout << "  ✓ 成功" << std::endl << std::endl;
	}
	...
	std::cout << "（仮想関数を介さないため、パフォーマンスが向上）" << std::endl << std::endl;
